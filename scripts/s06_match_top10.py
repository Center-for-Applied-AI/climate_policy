...
def main() -> None:
    """Main entry point for top-10 paragraph matching with word counts."""
    ref_paragraphs = read_paragraphs(DATA_DIR / REFERENCE_FILE)
    ref_word_counts = [word_count(p) for p in ref_paragraphs]
    ...
    for src_name in SOURCE_FILES:
        ...
        src_word_counts = [word_count(p) for p in src_paragraphs]
        src_embs = np.array(get_embeddings(src_paragraphs))

        for idx, (p, emb, wc) in enumerate(zip(src_paragraphs, src_embs, src_word_counts), start=1):
        ...
        data: dict[str, list] = {
            ...
            f"{src_stem}_paragraph_word_count": src_word_counts,
        }
        for k in range(TOP_K):
            data[f"final_paragraph_num_{k+1}"] = topk_idx[:, k] + 1
            data[f"final_paragraph_{k+1}"] = [ref_paragraphs[j] for j in topk_idx[:, k]]
            data[f"final_paragraph_word_count_{k+1}"] = [ref_word_counts[j] for j in topk_idx[:, k]]
    ...
    for j in range(sim_ref_to_all.shape[0]):
        record: dict[str, str | int | float] = {
            ...
            "final_paragraph_word_count": ref_word_counts[j],
        }
        for k in range(TOP_K):
            ...
            record[f"match{k+1}_source_paragraph_word_count"] = src_info["word_count"]
    ...
    output_path = f"outputs/final_top{TOP_K}_matches_from_sources.xlsx"
