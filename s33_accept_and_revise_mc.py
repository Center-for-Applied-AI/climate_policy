#!/usr/bin/env python3
"""Run Monte Carlo acceptance and revision sweeps with Gemini outputs."""
...
TRACE_XLSX_PATH: Path = Path("outputs/s33_role_trace_all_agencies_all_roles.xlsx")
SIM_XLSX_PATH: Path = Path("outputs/s33_role_similarity_all_agencies_all_roles.xlsx")
SIM_MATRIX_AVG_CSV: Path = Path("outputs/s33_policy_similarity_matrix_avg.csv")
HEATMAP_AVG_PNG: Path = Path("plots/s33_policy_similarity_heatmap_avg.png")
HEATMAP_AVG_SVG: Path = Path("plots/s33_policy_similarity_heatmap_avg.svg")
STACKED_PNG_PATH: Path = Path("plots/s33_similarity_stacked_official_final.png")
STACKED_SVG_PATH: Path = Path("plots/s33_similarity_stacked_official_final.svg")
T_MATRIX_CSV_PATH: Path = Path("outputs/s33_role_t_matrix.csv")
T_MATRIX_PNG_PATH: Path = Path("plots/s33_role_t_matrix_heatmap.png")
T_MATRIX_SVG_PATH: Path = Path("plots/s33_role_t_matrix_heatmap.svg")
ACCEPTANCE_RATES_MC_CSV: Path = Path("outputs/s33_acceptance_rates_by_role_mc.csv")
ACCEPTANCE_RATES_MC_PNG: Path = Path("plots/s33_acceptance_rates_with_ci.png")
ACCEPTANCE_RATES_MC_SVG: Path = Path("plots/s33_acceptance_rates_with_ci.svg")
SIMILARITY_RATES_MC_CSV: Path = Path("outputs/s33_similarity_to_final_by_role_mc.csv")
SIMILARITY_RATES_MC_PNG: Path = Path("plots/s33_similarity_to_final_with_ci.png")
SIMILARITY_RATES_MC_SVG: Path = Path("plots/s33_similarity_to_final_with_ci.svg")
...
def configure_gemini_client() -> None:
    ...
    -------
    None
        Configures genai library globally.
    """
    import os
...
def acceptance_messages(role_text: str, comment_text: str) -> str:
    """Construct prompt for the acceptance task.
...
def revision_messages(template_text: str, role_text: str, policy_text: str, proposals_block: str) -> str:
    """Construct prompt for single-shot revision using accepted proposals as comments.
...
def embed_texts(texts: List[str], desc: str = "Embeddings") -> List[Optional[List[float]]]:
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Monte Carlo acceptance and revisions (Gemini)."
    )
    parser.add_argument(
        "--agency",
        type=str,
...
def main() -> None:
    ...
    if plots_from_cache:
        ...
        if SIM_XLSX_PATH.exists():
            sim_all_df = pd.read_excel(SIM_XLSX_PATH)
        ...
        for seed_val in seeds_list:
            acc_path = Path(f"outputs/s33_comment_acceptance_all_roles_seed_{seed_val}.csv")
        ...
        if not role_similarity_mc_df.empty:
            ...
            plt.close()
            print(f"Saved similarity to final plot → {SIMILARITY_RATES_MC_PNG}, {SIMILARITY_RATES_MC_SVG}")
        ...
        if SIM_MATRIX_AVG_CSV.exists():
            ...
            plt.title("Average Cosine Similarity Between Policies Across Seeds (s33)")
            plt.tight_layout()
    ...
    ref_vecs = embed_texts(
        [original_final, original_draft, occ_draft_text, fdic_draft_text],
        desc="Embedding reference policies"
    )
    ...
    def run_acceptance_for_role(role_name: str, seed_val: int) -> List[Dict[str, object]]:
        ...
        acceptance_cache_path = Path(f"outputs/s33_acceptance_cache_seed_{seed_val}.json")
        ...
        def process_comment(cm: Dict[str, str]) -> Dict[str, object]:
            ...
            if cached_obj is not None:
                ...
                prompt = acceptance_messages(role_text=role_text, comment_text=cm_text)
                model = genai.GenerativeModel(
                    model_name=MODEL_NAME,
                    generation_config={
                        "response_mime_type": "application/json",
                        "response_schema": CommentAcceptanceOutput,
                    },
                )
                ...
                for attempt in range(MAX_RETRIES):
                    ...
                    parsed = CommentAcceptanceOutput.model_validate_json(response.text)
                    accepted_val = bool(parsed.accepted)
                    proposed_change_val = str(parsed.proposed_change)
                    break
    ...
    def run_revision_for_role(role_name: str, acceptance_df: pd.DataFrame) -> Tuple[Dict[str, object], Dict[str, object], Optional[List[float]]]:
        ...
        if accepted_df.empty:
            ...
            final_vec = embed_texts([final_policy_text], desc=f"Embedding {role_name} policy")[0]
        ...
        model = genai.GenerativeModel(
            ...
            generation_config={
                "response_mime_type": "application/json",
                "response_schema": BatchRevisionOutput,
            },
        )
        ...
        for attempt in range(MAX_RETRIES):
            wait_for_rate_limit()
            response = model.generate_content(prompt)
            parsed = BatchRevisionOutput.model_validate_json(response.text)
            revised_policy = parsed.revised_policy
            break

        proposals_words = word_count(proposals_block)
        final_vec = embed_texts([revised_policy], desc=f"Embedding {role_name} revised policy")[0]
        sim_final_vs_true_final = cosine_similarity(final_vec, ref_vec_true_final)
        sim_final_vs_original_draft = cosine_similarity(final_vec, ref_vec_orig_draft)
    ...
    for seed_val in tqdm(seeds_list, desc="Seeds"):
        ...
        acceptance_csv_path = Path(f"outputs/s33_comment_acceptance_all_roles_seed_{seed_val}.csv")
        ...
        for rn in tqdm(roles_to_run, desc=f"Revision [seed {seed_val}]"):
